...
/* *********************************************************************************************
   ...
   **********************************************************************************************
   *  /includes/skinParser.php                                                                  *
   *  Version 0.1                                                                               *
   *  Developed by Matt Howard and Phil Lopreiato                                               *
   *  OmniPage skin support                                                                     *
   **********************************************************************************************/

function parseSkin($fields,$loc,$cases = array()){ //Receives fields to replace as indexed array and name of template, returns parsed template
...
function getGhost($skin,$file){
	...
	if(file_exists($root_path."/skins/".$skinData['path']."/".$file.".html")){
		...
		return file_get_contents("http://uberbots.org/omni/error.php?errorCode=404");
	}
}
