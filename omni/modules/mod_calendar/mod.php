...
class mod_calendar {
	...
	public function render($properties) {
		...
		//view event page
		if(isset($_GET['viewEvent']) && isset($_GET['eventId']) && is_numeric($_GET['eventId']) && $properties['type']==0){ //check for GET variables
			unset($rep);
			unset($ifs);
			$q = mysql_query("SELECT * FROM `calendar` WHERE id = ".mysql_real_escape_string($_GET['eventId']));
			$invalid = mysql_num_rows($q) == 0;
			...
			$ifs = array("MINUTE"=>$this->minutes,"MENTORS"=>$this->mentors,"mentorCheck"=>$ismentor,"mentorAttending"=>$mentorAttending,"INVALID"=>$invalid);
			return parseSkin($rep,'calViewEvent',$ifs);
		}
		
		//view day
		if(isset($_GET['viewDay']) && isset($_GET['day']) && is_numeric($_GET['day']) && $properties['type']==0){
			$events = "";
			$rep = $ifs = array();
			$q = mysql_query("SELECT * FROM `calendar` WHERE (`startTime` >= '".mysql_real_escape_string($_GET['day'])."' AND `startTime` < '".(mysql_real_escape_string($_GET['day']) + (60*60*24))."') OR (`endTime` >= '".mysql_real_escape_string($_GET['day'])."' AND `endTime` < '".(mysql_real_escape_string($_GET['day']) + (60*60*24))."') OR (`startTime` < '".mysql_real_escape_string($_GET['day'])."' AND `endTime` >= '".(mysql_real_escape_string($_GET['day']) + (60*60*24))."')");
			$eventRepeat = 0;
			$lastId= 0;
			while($a = mysql_fetch_array($q)){
				...
				$mentorQuery = mysql_query("SELECT * FROM `calendarMentors` WHERE eventId = '".mysql_real_escape_string($_GET['eventId'])."' AND response = '1'");
				$numMentors = mysql_num_rows($mentorQuery);
				$this->mentors = ($numMentors>0 && $user->data['user_id'] != ANONYMOUS);
				...
				$mentorAttending = ($user->data["group_id"]==12 && $n>0)?true:false;
				$minURL = urlencode(date("D n-j-y",$a['startTime']));
				$startTime = date("D n-j-y \a\t g:i A",$a['startTime']); //date string of start time 
				$endTime = date('D n-j-y \a\t g:i A', $a['endTime']); //date string end time
				$events .= "<li>".parseSkin(array("eventId"=>$a['id'], "eventName"=>stripslashes($a['name']), "eventDescription"=>stripslashes($a['description']), "eventType"=>$a['type'], "startTime"=>$startTime, "endTime"=>$endTime, "eventLocation"=>stripslashes($a['location']), "mentors"=>$mentorList, "minutes"=>$minute, "minuteURL"=>$a->minuteURL,"suffix"=>$eventRepeat), "calEvent", array("minutes"=>$this->minutes, "loggedIn"=>$loggedIn, "list"=>TRUE,"VIEW_DAY"=>true,"multiple"=>$eventRepeat > 0))."</li>";
				$lastId = $a['id'];
			}
			$rep = array('events'=>$events,'day'=>date('F j, Y',$_GET['day']));
			return parseSkin($rep,'calViewDay');
		}
			...
			if(mysql_num_rows(mysql_query("SELECT * FROM `uberbots_omni`.`calendar` WHERE (`startTime` >= '".$currentDate."' AND `startTime` < '".($endDate + (60*60*24))."')"))!=0){
				...
				for($currentDate; $currentDate <= $endDate; $currentDate += (60*60*24)){
					...
					if(mysql_num_rows($query)>0){
						...
						$list .= "<li>".parseSkin(array("eventId"=>$row['id'], "eventName"=>stripslashes($row['name']), "eventDescription"=>stripslashes($row['description']), "eventType"=>$row['type'], "startTime"=>$startTime, "endTime"=>$endTime, "eventLocation"=>stripslashes($row['location']), "mentors"=>$mentors, "minutes"=>$minute, "minuteURL"=>$this->minuteURL,"suffix"=>floor(($row['startTime'] - $currentDate)/86400)), "calEvent", array("minutes"=>$this->minutes, "loggedIn"=>$loggedIn, "list"=>TRUE,"VIEW_DAY"=>true,"multiple"=>($row['endTime']-$row['startTime'] >= 86400),"multiple"=>$row['endTime']-$row['startTime']>86400))."</li>";
					}	
					if($e)
						$list .= "</ul>";
				}
		...
		for($day_counter = 1; $day_counter <= $days_in_this_month; $day_counter++){ //this for loop outptus days of the month
				...
				*/
				...
				while($row = mysql_fetch_array($oneDay)){
					...
					$this->minuteURL = urlencode(date("D n-j-y",$row['startTime']));
					$loggedIn = ($user->data['user_id'] != ANONYMOUS);
					$event .= parseSkin(array("eventId"=>$row['id'], "eventName"=>stripslashes($row['name']), "eventDescription"=>stripslashes($row['description']), "eventType"=>$row['type'], "startTime"=>$startTime, "endTime"=>$endTime, "eventLocation"=>stripslashes($row['location']), "mentors"=>$mentors, "minutes"=>$minute, "minuteURL"=>$this->minuteURL,"suffix"=>floor(($thisUnix - $row['startTime'])/86400)), "calEvent", array("minutes"=>$this->minutes, "loggedIn"=>$loggedIn, "list"=>FALSE,"VIEW_DAY"=>false,"multiple"=>$row['endTime']-$row['startTime']>86400));
				...
				$output .= parseSkin(array("click"=>$click,"month"=>$month, "day"=>$day, "year"=>$year, "class"=>$dayClass, "event"=>$event,"time"=>mktime(0,0,0,$month,$day,$year)), "calDay", array("today"=>$todayCheck, "ADMIN"=>$pagePermissions,"NOT ADMIN"=>!$pagePermissions,"event"=>($dayClass=="event")));
