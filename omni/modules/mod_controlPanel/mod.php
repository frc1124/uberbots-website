...
class mod_controlPanel {
	...
	public function render($properties) {
		...
		switch($_POST['mode']){
			...
			case "editPage":
				$message = "";
				$pageId = mysql_real_escape_string($_POST['pageSelect']);
				$newTitle = mysql_real_escape_string($_POST['newTitle']);
				$newParent = mysql_real_escape_string($_POST['newParent']);
				$newOrder = mysql_real_escape_string($_POST['newMenuOrder']);
				$newInh = $_POST["newInherent"]=="on"?1:0;
				$newPrivate = ($_POST["newPrivate"]=="on"?1:0);
				$newHide = ($_POST['newHideBox']=='on'?1:0);
				$newRedirect = mysql_real_escape_string($_POST["newRedirect"]);
				
				$query = mysql_query("UPDATE `pages` SET `title` = '".$newTitle."', `parentId` = '".$newParent."', `order` = '".$newOrder."', `inheritPermissions` = '".$newInh."', `private` = '".$newPrivate."', `hide` = '".$newHide."', `redirect` = '".$newRedirect."' WHERE `id`='".mysql_real_escape_string($pageId)."'");
				if($query){
					$message = "Page Updated Sucessfully";
					//add log entry for page update
					logEntry("updated page, ID= ".$pageId."");
				}else{
					$message = "Page Updating Failed ".mysql_error();
				}
				break;
							...
							$permissionType = mysql_real_escape_string($_POST['permissionType']);
									...
									switch($type){
										...
										case "addPermission":
											...
											$test = mysql_query("SELECT * FROM `pagePermissions` WHERE `name` = '".$user."' AND `pageId` = '".$pageId."'",$GLOBALS["mySQLLink"]);
											if(mysql_fetch_array($test)){
												...
												$insert = mysql_query("INSERT INTO  `uberbots_omni`.`pagePermissions` (`id` ,`name` ,`pageId` ,`access`)VALUES ('".$userId."', '".$user."' , '".$pageId."', '".$permissionType."');",$GLOBALS["mySQLLink"]) or die(mysql_error());
										...
										case "editPermission":
											...
											$update = mysql_query("UPDATE pagePermissions SET access = '".$permissionType."' WHERE name='".$user."' AND pageId='".$permissionType."'",$GLOBALS["mySQLLink"]);
											$message .= $update?"Permissions updated sucessfully.":mysql_error();
											//add log entry
											logEntry("Updated permissions for user ".$user." on page ID ".$pageId." to access ".$permissionType."");
											break;
										
										//delete permissions
										case "delPermission":
											//delete permissions from SQL
											$del = mysql_query("DELETE FROM `pagePermissions` WHERE `name` = '".$user."' AND `pageId` = '".$pageId."'",$GLOBALS["mySQLLink"]);
		...
		$groups = "";
		//$query = $db->sql_query("SELECT * FROM `phpbb_groups`",$GLOBALS["mySQLLink"]);
		/*while($row = $db->sql_fetchrow($query)){
			$groups .= "<option id='".$row['group_id']."'>".$row['group_name']."</option>";
		}*/
		...
		return parseSkin(array('skins'=>$skins,"fullChildren"=>$this->allPages,"children"=>$this->selectOutput,"message"=>$message,"log"=>$log,"groups"=>$groups),'controlPanel',array("MESSAGE"=>isset($message)));
